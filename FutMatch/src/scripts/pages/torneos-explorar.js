...
function aplicarFiltros() {
    ...
    const estadosSeleccionados = Array.from(document.querySelectorAll('input[type="checkbox"]:checked'))
    ...
    if (premioMinimo || premioMaximo) {
        const rangoPremio = `Premio: ${premioMinimo || '0'} - ${premioMaximo || '∞'}`;
    ...
    filtrarPorFiltros(estadosSeleccionados, zonaSeleccionada, premioMinimo, premioMaximo);
...
function filtrarPorFiltros(estados, zona, premioMin, premioMax) {
    ...
    torneos.forEach(torneo => {
        ...
        // Filtro por zona
        if (zona) {
            const ubicacionTorneo = torneo.getAttribute('data-ubicacion').toLowerCase();
...
function verDetalleTorneo(id) {
    ...
    window.location.href = `torneoDetalle.php?id=${id}`;
}
...
function inicializarMapa() {
    if (typeof L === 'undefined') {
        console.error('Leaflet no está cargado. Necesitas incluir la librería de Leaflet.');
        return;
    }

    const map = L.map('map').setView([-34.6118, -58.3960], 12); // Buenos Aires
