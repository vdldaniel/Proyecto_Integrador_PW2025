...
const cargarSelectores = () => {
    ...
    return Promise.all([
        ...
        fetch(BASE_URL + "src/controllers/admin-cancha/get_tipos_partido.php")
            .then(r => r.json())
            .then(json => {
                TIPOS_PARTIDO_CACHE = json.data || [];
                const select = document.getElementById("editCapacidadCancha");
                // Importante: Limpiar y rellenar el select
                select.innerHTML = '<option value="">Seleccione tipo de cancha</option>';
...
function abrirModalEditar(id) {
    ...
    if (!cancha) {
        console.error("No se encontró la cancha con ID", id, "en la caché. Cargue el perfil primero.");
        mostrarToast("No se pudo cargar la información de la cancha para editar. Inténtelo de nuevo.", false);
        return;
    }
    ...
    document.getElementById("editUbicacionCancha").value = cancha.direccion_completa || cancha.ubicacion || ""; 
    
    // **CORRECCIÓN CLAVE 2: Revisar la propiedad de Descripción.**
    // Intenta con las diferentes propiedades que aparecen en el perfil:
    document.getElementById("editDescripcionCancha").value = cancha.descripcion || cancha.descripcion_cancha || "";
    ...
    const idTipoPartidoActual = cancha.id_tipo_partido 
        || (cancha.tipos_partido && cancha.tipos_partido.length > 0 ? cancha.tipos_partido[0].id_tipo_partido : "");
    ...
    fetch(BASE_URL + "src/controllers/admin-cancha/update_cancha.php", {
        ...
        .then(res => {
            if (res.status === "success") {
                ...
                console.error("Error del servidor al actualizar:", res.message || res);
                mostrarToast("Error al actualizar la cancha: " + (res.message || "Error desconocido"), false); // Mensaje de error
            }
        })
        .catch(err => {
...
function actualizarBanner(id) {
    ...
    fetch(BASE_URL + "src/controllers/admin-cancha/get_perfil_cancha.php?id=" + id)
        ...
        .then(json => {
            ...
            const servicios = Array.isArray(json.data.servicios) ? json.data.servicios : [];
            const idEstado = parseInt(cancha.id_estado);
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    cargarSelectores()
        .then(() => {
            console.log("Selectores de edición (Superficie, Capacidad) cargados y listos.");
        ...
        .catch(error => {
            console.error("Error crítico al inicializar la aplicación. No se pudieron cargar los selectores de edición.", error);
            mostrarToast("Error crítico de inicialización. Recargue la página.", false);
        });
});