  ...
  canchasConUbicacion.forEach((cancha) => {
    ...
    const popupContent = `
            <div class="popup-cancha">
                ...
                <div class="mb-2">
                    <span class="badge text-bg-dark">${cancha.tipo}</span>
                    <span class="badge text-bg-dark">${cancha.superficie}</span>
                </div>
                <p class="mb-1"><strong>${cancha.precio}</strong></p>
                <div class="mb-2">
                    <div class="text-warning">
                        ${"★".repeat(Math.floor(cancha.rating))}${"☆".repeat(
      5 - Math.floor(cancha.rating)
    )}
                ...
                <p class="mb-2">
                    <span class="badge ${
                      cancha.disponible ? "bg-success" : "bg-danger"
                    }">
                        ${cancha.disponible ? "Disponible" : "No disponible"}
                    </span>
                </p>
                <button class="btn btn-sm btn-primary" onclick="verDetalleCancha(${
                  cancha.id
                })">
                    Ver detalles
                </button>
...
function obtenerFiltrosSeleccionados() {
  ...
  const checkboxesTipos = document.querySelectorAll(
    'input[type="checkbox"][id^="futbol"], input[type="checkbox"][id="futbolSala"]'
  );
  ...
  // Superficies
  const checkboxesSuperficies = document.querySelectorAll(
    '#modalFiltros input[type="checkbox"][id^="sintetico"], #modalFiltros input[type="checkbox"][id^="cemento"], #modalFiltros input[type="checkbox"][id^="parquet"], #modalFiltros input[type="checkbox"][id^="cespedNatural"]'
  );
...
function filtrarCanchas() {
  const termino = document
    .getElementById("busquedaCanchas")
    .value.toLowerCase()
    .trim();
  ...
  canchas.forEach((cancha) => {
    ...
    if (
      ...
      !filtros.superficies.includes(superficie)
    ) {
      mostrar = false;
    }
...
function reservarCancha(idCancha) {
  ...
  mostrarToast(
    `Iniciando proceso de reserva para cancha ${idCancha}`,
    "success"
  );
}
