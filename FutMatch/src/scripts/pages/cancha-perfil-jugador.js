...
class PerfilCanchaJugador extends PerfilCanchaBase {
    ...
    configurarEventListenersJugador() {
        ...
        const btnCalendarioCompleto = document.getElementById('btnVerCalendarioCompleto');
        if (btnCalendarioCompleto) {
            btnCalendarioCompleto.addEventListener('click', () => this.irACalendario());
        }
    }
    ...
    inicializarReservas() {
        ...
        if (fechaReserva) {
            ...
            fechaReserva.addEventListener('change', () => this.actualizarHorariosDisponibles());
        }
    ...
    notificarInicio(torneoId) {
        ...
        this.mostrarNotificacion('Te notificaremos cuando inicie el torneo', 'success');
    }
    ...
    abrirModalReserva(event) {
        ...
        this.mostrarNotificacion(`Reservando para ${this.formatearFecha(fecha)} a las ${hora}`, 'info');
     ...
     */
    irACalendario() {
        // La URL ya está configurada en el componente base como PAGE_CALENDARIO_CANCHA
