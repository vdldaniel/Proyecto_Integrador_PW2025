...
function mostrarDetallePartido(cardElement) {
    ...
    console.log(`Mostrando detalles del partido: ${partidoId} - ${equipo1} vs ${equipo2}`);
}
...
function actualizarBadgesGoles(goles1, goles2) {
    ...
    if (goles1 === '-' || goles2 === '-') {
        // Partido pendiente
        badge1.className = 'badge bg-secondary fs-4';
        badge2.className = 'badge bg-secondary fs-4';
        document.getElementById('modal-estado').innerHTML = '<span class="badge bg-warning">Pendiente</span>';
    } else {
        // Partido finalizado
        ...
        if (g1 > g2) {
            badge1.className = 'badge bg-success fs-4';
            badge2.className = 'badge bg-secondary fs-4';
        } else if (g2 > g1) {
            badge1.className = 'badge bg-secondary fs-4';
            badge2.className = 'badge bg-success fs-4';
        } else {
            badge1.className = 'badge bg-warning fs-4';
            badge2.className = 'badge bg-warning fs-4';
        }
        
        document.getElementById('modal-estado').innerHTML = '<span class="badge bg-success">Finalizado</span>';
    }
}
...
function actualizarInfoAdicional(partidoId, goles1) {
    ...
    document.getElementById('modal-amarillas').textContent = partidoInfo.amarillas;
...
function obtenerInfoPartido(partidoId) {
    ...
    return partidosConfig[partidoId] || {
        fecha: 'Por definir',
        hora: 'Por definir',
        cancha: 'Por definir',
