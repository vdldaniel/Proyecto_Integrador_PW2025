...
const reservasSimuladas = [
    // Noviembre 2025
    { fecha: '2025-11-04', hora: '18:00', duracion: 2, estado: 'reservado', cliente: 'Los Cracks FC' },
    { fecha: '2025-11-04', hora: '20:00', duracion: 2, estado: 'reservado', cliente: 'Equipo Delta' },
    { fecha: '2025-11-05', hora: '19:00', duracion: 1, estado: 'reservado', cliente: 'Racing Amateur' },
    { fecha: '2025-11-05', hora: '21:00', duracion: 2, estado: 'reservado', cliente: 'Villa FC' },
    { fecha: '2025-11-06', hora: '17:00', duracion: 3, estado: 'reservado', cliente: 'Deportivo Sur' },
    { fecha: '2025-11-07', hora: '18:00', duracion: 2, estado: 'reservado', cliente: 'Club Atlético' },
    { fecha: '2025-11-08', hora: '20:00', duracion: 1, estado: 'reservado', cliente: 'Los Tigres' },
    { fecha: '2025-11-09', hora: '16:00', duracion: 2, estado: 'reservado', cliente: 'Torneo Copa Verano' },
    { fecha: '2025-11-09', hora: '18:00', duracion: 2, estado: 'reservado', cliente: 'Torneo Copa Verano' },
    { fecha: '2025-11-09', hora: '20:00', duracion: 2, estado: 'reservado', cliente: 'Torneo Copa Verano' },
    { fecha: '2025-11-10', hora: '15:00', duracion: 4, estado: 'no-disponible', motivo: 'Mantenimiento' }
];
...
function configurarEventListeners() {
    ...
    document.getElementById('botonAnterior')?.addEventListener('click', navegarAnterior);
    document.getElementById('botonSiguiente')?.addEventListener('click', navegarSiguiente);
    ...
    document.getElementById('botonReservarHorario')?.addEventListener('click', function() {
    ...
    document.getElementById('duracionReservaRapida')?.addEventListener('change', calcularTotalReservaRapida);
}
...
function irAHoy() {
    ...
    document.getElementById('selectorFecha').value = fechaActual.toISOString().split('T')[0];
    actualizarVistaActual();
}

function cambiarVista(nuevaVista) {
    vistaActual = nuevaVista;
    document.getElementById('vistaActual').textContent = nuevaVista.charAt(0).toUpperCase() + nuevaVista.slice(1);
    ...
    const vistaElemento = document.getElementById(`vista${nuevaVista.charAt(0).toUpperCase() + nuevaVista.slice(1)}`);
    if (vistaElemento) {
...
function actualizarTituloFecha() {
    ...
    if (vistaActual === 'mes') {
        textoFecha = `Calendario - ${fechaActual.toLocaleDateString('es-ES', opciones)}`;
    } else if (vistaActual === 'semana') {
        ...
        textoFecha = `Calendario - Semana del ${inicioSemana.getDate()} al ${finSemana.getDate()} de ${fechaActual.toLocaleDateString('es-ES', { month: 'long', year: 'numeric' })}`;
    } else if (vistaActual === 'dia') {
        textoFecha = `Calendario - ${fechaActual.toLocaleDateString('es-ES', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}`;
    }
    ...
    for (let semana = 0; semana < 6; semana++) {
        ...
        for (let dia = 0; dia < 7; dia++) {
            ...
            if (disponibilidad.total === 0) {
                indicador.innerHTML = '<span class="badge bg-secondary">Cerrado</span>';
            } else if (disponibilidad.disponibles === 0) {
                indicador.innerHTML = '<span class="badge bg-danger">Sin horarios</span>';
            } else if (disponibilidad.disponibles <= 3) {
                indicador.innerHTML = `<span class="badge bg-warning text-dark">${disponibilidad.disponibles} horarios</span>`;
            } else {
                indicador.innerHTML = `<span class="badge bg-success">${disponibilidad.disponibles} horarios</span>`;
            }
    ...
    for (let hora = 7; hora <= 23; hora++) {
        ...
        for (let dia = 0; dia < 7; dia++) {
            ...
            if (disponible) {
                ...
                celda.innerHTML = '<div class="text-center small text-success">Disponible</div>';
                ...
                if (reserva) {
                    ...
                    celda.innerHTML = `<div class="text-center small text-danger">${reserva.cliente || 'Reservado'}</div>`;
                } else {
                    celda.classList.add('bg-secondary', 'bg-opacity-25');
                    celda.innerHTML = '<div class="text-center small text-muted">No disponible</div>';
                }
            }
...
function renderizarVistaDiaria() {
    ...
    for (let hora = 7; hora <= 23; hora++) {
        ...
        const celdaHora = document.createElement('td');
        celdaHora.classList.add('text-center', 'fw-bold', 'bg-light', 'text-dark');
        celdaHora.textContent = `${hora.toString().padStart(2, '0')}:00 - ${(hora + 1).toString().padStart(2, '0')}:00`;
        fila.appendChild(celdaHora);
        ...
        if (disponible) {
            ...
            celda.innerHTML = `
                <div class="d-flex justify-content-between align-items-center p-2">
                    ...
                    <button class="btn btn-sm btn-success" onclick="seleccionarHorario('${formatearFecha(fechaActual)}', '${horaString}')">
...
function actualizarResumenDia() {
    ...
    resumen.innerHTML = `
        <div class="text-center mb-3">
            <h5>${fechaActual.toLocaleDateString('es-ES', { weekday: 'long', day: 'numeric', month: 'long' })}</h5>
        </div>
...
function esHorarioDisponible(fecha, hora) {
    ...
    if (fecha < hoy.toDateString() || (fecha.toDateString() === hoy.toDateString() && parseInt(hora) <= hoy.getHours())) {
        return false;
    }
    
    // Verificar si ya está reservado
    return !reservasSimuladas.some(reserva => 
        reserva.fecha === fechaString && 
        reserva.hora === hora
    );
}
...
function obtenerReserva(fecha, hora) {
    const fechaString = formatearFecha(fecha);
    return reservasSimuladas.find(reserva => 
        reserva.fecha === fechaString && 
        reserva.hora === hora
    );
}
...
function mostrarModalReservaRapida() {
    ...
    document.getElementById('horarioSeleccionado').textContent = `${horarioSeleccionado} - ${parseInt(horarioSeleccionado.split(':')[0]) + 1}:00`;
    
    const fechaObj = new Date(fechaSeleccionada);
    document.getElementById('fechaSeleccionada').textContent = fechaObj.toLocaleDateString('es-ES', {
        weekday: 'long',
        year: 'numeric',
        month: 'long',
        day: 'numeric'
...
function calcularTotalReservaRapida() {
    const duracion = parseInt(document.getElementById('duracionReservaRapida')?.value) || 2;
    ...
    document.getElementById('subtotalReservaRapida').textContent = `$${subtotal.toLocaleString()}`;
    document.getElementById('descuentoReservaRapida').textContent = `-$${descuento.toLocaleString()}`;
    document.getElementById('totalReservaRapida').textContent = `$${total.toLocaleString()}`;
}
...
function confirmarReservaRapida() {
    ...
    const modal = bootstrap.Modal.getInstance(document.getElementById('modalReservaRapida'));
    modal.hide();
