...
class AplicacionCalendarioJugador extends CalendarioBase {
    ...
    personalizarCalendarioJugador() {
        ...
        const espacioLibre = document.querySelector('.calendario-header .col-md-3:nth-child(3)');
        if (espacioLibre) {
                ...
                if (hora.includes(':00')) {
                    ...
                    if (this.esHorarioDisponible(this.fechaActual, horaCompleta)) {
                        celdaContenido.innerHTML = `
                            <button class="btn btn-outline-success btn-sm w-100 btnSeleccionarHorario" 
                                    data-fecha="${this.formatearFecha(this.fechaActual)}" data-hora="${horaCompleta}">
    ...
    esHorarioDisponible(fecha, hora) {
        ...
        return !this.reservasSimuladas.some(reserva => 
    ...
    obtenerReserva(fecha, hora) {
        const fechaString = this.formatearFecha(fecha);
        return this.reservasSimuladas.find(reserva => 
            reserva.fecha === fechaString && 
            reserva.hora === hora
        );
    }
    ...
    seleccionarHorario(fecha, hora) {
        ...
        console.log('Horario seleccionado:', this.fechaSeleccionada, this.horarioSeleccionado);
        
        // Abrir modal de reserva directamente con fecha y hora prellenados
        this.mostrarModalReservarCancha(this.fechaSeleccionada, this.horarioSeleccionado);
    }
    ...
    enviarSolicitudReserva() {
        ...
        const modal = bootstrap.Modal.getInstance(document.getElementById('modalReservarCancha'));
        modal.hide();

        // Mostrar mensaje de éxito
        setTimeout(() => {
            alert('¡Solicitud enviada con éxito! El administrador de la cancha revisará tu solicitud y te contactará pronto.');
