...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Modal Editar Equipo
  const btnAgregarJugadorEdit = document.getElementById(
    "btnAgregarJugadorEdit"
  );
  ...
  const btnGuardarEdicionEquipo = document.getElementById(
    "btnGuardarEdicionEquipo"
  );
...
function crearEquipo(formData) {
  ...
  alert(
    `Creando equipo: ${formData.nombre} con ${formData.jugadores.length} jugadores`
  );
  bootstrap.Modal.getInstance(
...
function inicializarTooltipsModal() {
  ...
  const tooltips = document.querySelectorAll(
    '#modalEditarEquipo [data-bs-toggle="tooltip"]'
  );
  tooltips.forEach((tooltip) => {
    new bootstrap.Tooltip(tooltip);
  });
...
function recopilarDatosFormularioEdit() {
  ...
  const jugadores = Array.from(
    document.querySelectorAll('[name="jugadorEdit[]"]')
  )
    .map((input) => input.value.trim())
    .filter((value) => value.length > 0);
...
function editarEquipo(formData) {
  ...
  alert(
    `Guardando cambios del equipo. Jugadores: ${formData.jugadores.join(", ")}`
  );
  bootstrap.Modal.getInstance(
...
function irADetalleEquipo(equipoId) {
  ...
  const baseUrl =
    ...
    window.location.pathname.replace(
      "equiposListado_Jugador.php",
      "perfilEquipo_Jugador.php"
    );
